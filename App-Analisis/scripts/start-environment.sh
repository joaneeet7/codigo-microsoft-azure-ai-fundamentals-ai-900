...
wait_for_backend() {
  node - <<'NODE'
const url = "http://localhost:3010/health";
...
async function waitForBackend() {
  while (Date.now() < deadline) {
    try {
      ...
      if (response.ok) {
        console.log("[ok] Backend listo en http://localhost:3010");
        return;
      }
    } catch {
...
ensure_python() {
  ...
  if command -v python >/dev/null 2>&1 && python -c "import sys" >/dev/null 2>&1; then
    ...
    fail "No encontre Python funcional. Instala Python 3 o habilita el launcher py."
  fi
}
...
ensure_python_pip() {
  ...
  if [[ "$PYTHON_CMD" != "py" ]] && command -v py >/dev/null 2>&1 && py -m pip --version >/dev/null 2>&1; then
    PYTHON_CMD="py"
    return
  fi
  ...
  if "$PYTHON_CMD" -m ensurepip --upgrade >/dev/null 2>&1; then
    ok "pip activado correctamente"
    return
  fi

  if [[ "$PYTHON_CMD" != "py" ]] && command -v py >/dev/null 2>&1 && py -m ensurepip --upgrade >/dev/null 2>&1; then
    PYTHON_CMD="py"
    ok "pip activado correctamente usando py"
    return
  fi

  fail "Tu Python no tiene pip y no pude activarlo. Ejecuta: py -m ensurepip --upgrade"
}

ensure_python_dependencies() {
  if "$PYTHON_CMD" -c "import fastapi, uvicorn, dotenv, requests" >/dev/null 2>&1; then
...
run_terraform() {
  ...
  if [[ "$write_env" =~ ^[sS]$ ]]; then
    ...
    info "No se modifico backend/.env. Copia el bloque anterior manualmente si quieres usar Azure."
  fi
}
